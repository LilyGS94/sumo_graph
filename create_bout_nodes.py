...
class AuraDBLoader:
    ...
    def create_bout_node(
        ...
        with self.driver.session() as session:
            ...
            query = (
                "MERGE (b:Bout {result: $result,rikishiID: $RikishiID,rikishiSide: $Side, fightNumber: $Fight_Number,"
                "kimarite: $kimarite, opponentID: $opponentID, opponentShikonaEn: $opponentShikonaEn}) "
                "RETURN b"
            )  # TODO: Change to create bout node
            result = session.run(
                query,
                result=result,
    ...
    def create_basho_bout_relationship(self, name, age, date):
        with self.driver.session() as session:
            query = """
            MATCH (p:Person {name: $name, age: $age})
            MATCH (d:Date {date: $date})
            CREATE (p)-[:RECORDED_ON]->(d)
            """
            result = session.run(query, name=name, age=age, date=date)
            return result.single()[0]
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith(".json"):
                ...
                with open(file_path) as file:
                    ...
                    east_records = df_east["record"]
                    west_records = df_west["record"]
                    ...
                    east_rikishi_ids = [
                        ...
                        for rikishi_id, record in zip(
                            df_east["rikishiID"], df_east["record"]
                        )
                    ]
                    west_rikishi_ids = [
                        [rikishi_id] * len(record)
                        for rikishi_id, record in zip(
                            df_west["rikishiID"], df_west["record"]
                        )
                    ]
